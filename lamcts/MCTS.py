...
class MCTS:
    ...
    def __init__(self, lb, ub, dims, ninits, func, func1,Cp = 150, leaf_size = 25, kernel_type = "rbf", gamma_type = "auto"):
        self.dims                    =  dims
        self.samples                 =  []
        self.nodes                   =  []
        self.Cp                      =  Cp
    ...
    def search(self, iterations):
        ...
        kernel = Matern()
        gp_model = GPR(kernel=kernel, alpha=1e-4, n_restarts_optimizer=100, normalize_y=False)
        gpc_model = GPC(kernel=kernel, n_restarts_optimizer=100)

        opt_idx = np.argmax(fX)
        opt_x = X[opt_idx]
        opt_y = fX[opt_idx]
        opt_ys = [opt_y]
        
        bounds = np.array([0,1]*72).reshape(72,2)
        ...
        for idx in range(10,60):
            ...
            print("iteration:", idx)
            print("="*10)
            ys_normalized = normalize(ys)
            gp_model.fit(xs.reshape(-1,72), ys_normalized)
        ...
        # Decide point to evaluate next
            n_candidates = 500*dim
            x = sample_next_point(dim, constraint_weighted_acquisition_func, bounds=bounds, strict_bounds=True, n_candidates=n_candidates,random_search=True)
            ...
            opt_y=max(ys)
            opt_x=xs[np.argmax(ys)]
            opt_ys.append(opt_y) # Best performance so far
            
            samples=np.array(x, ndmin=2)
            value = self.collect_samples(samples[0])
        ...
        for idx in range(60, iterations):
            print("")
            print("="*10)
            print("iteration:", idx)
            print("="*10)
