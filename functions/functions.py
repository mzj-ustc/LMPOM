...
class tracker:
    ...
    def dump_trace(self):
        ...
        final_results_str = json.dumps(self.results,ensure_ascii=False, cls=NpEncoder)
...
class myFunc:
    ...
    def __call__(self, x):
        ...
        t=torch.tensor(x).reshape(-1,72)
        truss.apply_discrete_thres(t)
        
        outputs=func1(t)
        result=1 / outputs.item()
...
class myFunc1:
    def __init__(self, dims=72):
        ...
        self.lb      =  np.zeros(dims)         #lower bound for each dimensions 
        self.ub      =  np.ones(dims)         #upper bound for each dimensions 
        self.tracker = tracker('myFunc1')      #defined in functions.py
